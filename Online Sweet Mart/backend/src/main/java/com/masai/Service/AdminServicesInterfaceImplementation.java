...
@Service
@Slf4j
public class AdminServicesInterfaceImplementation implements AdminServicesInterface {
	@Autowired
...
//	private PasswordEncoder pass;
	...
	public Admin addAdmin(Admin admin) throws AdminException {
		...
		if(opt.isPresent() && opt.get().getActive()) {
			throw new AdminException("Already Exists with these credentials...");
		}
		if(admin.getAdminId()!=0) {
			throw new AdminException("Id is AutoGenerated Type Don't mention it...");
		}
		if(admin==null) throw new AdminException("Fields don't have any data");
//		admin.setPassword(pass.enCode(admin.getPassword()));
	...
	@Override
	public Admin updateAdmin(Integer adminId,Admin admin) throws AdminException {
		...
		if(opt.isEmpty() || !opt.get().getActive()) {
			throw new AdminException("No Admin Found with this ID...");
		}
		if(admin.getAdminId()!=0) {
			throw new AdminException("Id is AutoGenerated Type Don't mention it...");
		}
...
//		opt.get().setPassword(pass.enCode(admin.getPassword()));
	...
	public Admin deleteAdmin(Integer adminId)throws AdminException {
		...
		Optional<Admin> opt = aRepo.findById(adminId);
		if(opt.isEmpty()  ||  !opt.get().getActive()) {
			throw new AdminException("No Admin Found with these Details...");
		}
	...
	@Override
	public Customer deleteCustomer(Integer customerId) throws AdminException {
		log.info("Admin is Deleting a Customer in Service Layer");
		Optional<Customer> opt = cuRepo.findById(customerId);
		if(opt.isEmpty() || !opt.get().getActive()) {
			throw new AdminException("No Customer Found with these Details...");
		}
		opt.get().setActive(false);
		cuRepo.save(opt.get());
		return opt.get();
	...
	@Override
	public Product addProduct(int cateId,Product product) throws AdminException {
		log.info("Admin is Adding Product in Service Layer");
		Optional<Product> opt = pRepo.findById(product.getProductId());
		if(opt.isPresent() && opt.get().getActive()) {
			throw new AdminException("Already Exists with these Details...");
		}
		if(product.getProductId()!=0) {
			throw new AdminException("Id is AutoGenerated Type Don't mention it...");
		}
		if(product==null) throw new AdminException("Fields don't have any data");
		Optional<Category> optt = cRepo.findById(cateId);
		if(optt.isEmpty() || !optt.get().getActive()) {
			throw new AdminException("No Category Found with this ID...");
		}
	...
	@Override
	public Product updateProduct(Integer productId, Product product) throws AdminException {
		log.info("Admin is Updating Product in Service Layer");
		Optional<Product> opt = pRepo.findById(productId);
		if(opt.isEmpty() || !opt.get().getActive()){
			throw new AdminException("No Product Found with this ID...");
		}
		if(product.getProductId()!=0) {
			throw new AdminException("Id is AutoGenerated Type Don't mention it...");
		}
...
//		opt.get().setOrder(opt.get().getOrder());
	...
	@Override
	public Product deleteProduct(Integer productId) throws AdminException {
	...
	public Category addCategory(Category category) throws AdminException {
		...
		if(category==null) throw new AdminException("Fields don't have any data");
		cRepo.save(category);
		return category;
	}
	@Override
	public Category deleteCategory(Integer categoryId) throws AdminException {
	...
	@Override
	public Category updateCategory(Integer categoryId, Category category)throws AdminException {
...
//		opt.get().setProducts(opt.get().getProducts());
	...
	@Override
	public List<Category> getAllCategory()throws AdminException {
		List<Category>categories = cRepo.findAll();
		if(categories.size()==0) {
			throw new AdminException("No Category Found");
		}
	...
	@Override
	public List<Orders> getAllOrders() throws AdminException {
	...
	@Override
	public List<Orders> getAllOrdersOfCustomer(Integer customerId) throws AdminException {
		Optional<Customer> opt = cuRepo.findById(customerId);
		if(opt.isEmpty() || !opt.get().getActive()) {
			throw new AdminException("No Customer Found with these Details...");
		}
		if(opt.get().getCart()==null) {
			throw new AdminException("Cart of customer doesn't have any Order...");
		}
	...
	@Override
	public Orders deleteOrder(Integer orderId)throws AdminException {
		Optional<Orders>opt = oRepo.findById(orderId);
		if(opt.isEmpty() || !opt.get().getActive()) {
			throw new AdminException("No Order Found with this Id...");
		}
		opt.get().setActive(false);
		oRepo.save(opt.get());
		return opt.get();
...
//	}
	...
	@Override
	public Customer getCustomerByEmail(String email)throws AdminException {
	...
	@Override
	public Customer ChangeRoleCustomer(Integer customerId, Customer customer) throws AdminException {
		...
		if(!customer.getRole().toUpperCase().equals("ADMIN") && !customer.getRole().toUpperCase().equals("CUSTOMER")) {
			throw new AdminException("Role isn't a Valid one");
		}
	...
	@Override
	public List<Category> getListOfCategory() throws AdminException {
